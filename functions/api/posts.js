...
export async function onRequest(context) {
  try {
    ...
    // Kiểm tra xem Sheet1 đã tồn tại chưa và tạo nếu chưa có
    try {
      ...
      if (context.request.method === 'GET') {
        ...
        if (!data.values || data.values.length <= 1) {
          // Nếu chỉ có tiêu đề hoặc không có dữ liệu
          return new Response(JSON.stringify([]), {
            headers: { 'Content-Type': 'application/json' }
          });
        }
        
        // Kiểm tra xem dòng đầu tiên có phải là tiêu đề không
        const headerRow = data.values[0];
        const expectedHeaders = ['ID', 'Title', 'Content', 'ContentPart2', 'ContentPart3', 'ContentPartsCount', 'FeaturedImage', 'Category', 'Tags', 'Status', 'Date'];
        const hasCorrectHeaders = headerRow && headerRow.length >= expectedHeaders.length;
        
        // Nếu không có tiêu đề đúng, tạo tiêu đề
        if (!hasCorrectHeaders) {
          ...
          // Trả về mảng rỗng vì chưa có dữ liệu thực
          return new Response(JSON.stringify([]), {
            headers: { 'Content-Type': 'application/json' }
          });
        }
        ...
        // Chuyển đổi dữ liệu hàng thành đối tượng Post
        ...
        // Đảm bảo post có ngày tạo
        if (!post.date) {
          post.date = new Date().toISOString().split('T')[0];
        }
        
        // Chia nội dung thành các phần nhỏ hơn 50,000 ký tự
        const content = post.content || '';
        const MAX_CELL_LENGTH = 49000; // Giữ dưới giới hạn một chút
        const contentParts = [];
        
        for (let i = 0; i < content.length; i += MAX_CELL_LENGTH) {
          contentParts.push(content.substring(i, i + MAX_CELL_LENGTH));
        }
        ...
        try {
          ...
          if (!existingResponse.ok) {
            ...
            console.error('Error fetching existing IDs:', errorData);
            throw new Error(`Failed to check existing posts: ${existingResponse.status}`);
          }
          
          const existingData = await existingResponse.json();
          const ids = existingData.values || [];
          let existingRowIndex = -1;
          
          // Bỏ qua hàng đầu tiên (tiêu đề)
          for (let i = 1; i < ids.length; i++) {
            if (ids[i] && ids[i][0] === post.id) {
              existingRowIndex = i + 1; // +1 vì hàng đầu tiên là tiêu đề
              break;
            }
          }
          
          // Đảm bảo có tiêu đề đúng cho bảng
          // Kiểm tra xem bảng có tiêu đề chưa
          let headersResponse = await fetch(
          ...
          if (headersResponse.ok) {
            ...
            // Thay đổi định nghĩa bảng sheet để thêm tags
const expectedHeaders = ['ID', 'Title', 'Content', 'ContentPart2', 'ContentPart3', 'ContentPartsCount', 'FeaturedImage', 'Category', 'Tags', 'Status', 'Date'];

            
            if (!headersData.values || headersData.values[0].length < expectedHeaders.length) {
              // Cập nhật tiêu đề
              await fetch(
                `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Sheet1!A1:K1?valueInputOption=RAW`,
                {
          ...
          if (existingRowIndex > 1) {
            ...
            const updateResponse = await fetch(
              `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Sheet1!A${existingRowIndex}:K${existingRowIndex}?valueInputOption=RAW`,
              {
                method: 'PUT',
                headers: {
                  'Authorization': `Bearer ${token}`,
            ...
            if (!updateResponse.ok) {
              ...
              throw new Error(`Failed to update post (${updateResponse.status}): ${JSON.stringify(errorData)}`);
            }
          } else {
            // Thêm bài viết mới
            const appendResponse = await fetch(
              `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Sheet1!A:K:append?valueInputOption=RAW`,
              {
                method: 'POST',
                headers: {
                  'Authorization': `Bearer ${token}`,
            ...
            if (!appendResponse.ok) {
              ...
              console.error('Error appending post:', errorData);
              throw new Error(`Failed to append post (${appendResponse.status}): ${JSON.stringify(errorData)}`);
            }
          }
        } catch (error) {
